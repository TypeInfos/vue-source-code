var Vue = (function (exports) {
  ...
  const CREATE_BLOCK = `createBlock`;
  const CREATE_VNODE = `createVNode`;
  const RESOLVE_COMPONENT = `resolveComponent`;
  const RESOLVE_DIRECTIVE = `resolveDirective`;
  const APPLY_DIRECTIVES = `applyDirectives`;
  const RENDER_LIST = `renderList`;
  const RENDER_SLOT = `renderSlot`;
  const CREATE_SLOTS = `createSlots`;
  const TO_STRING = `toString`;
  const MERGE_PROPS = `mergeProps`;
  const TO_HANDLERS = `toHandlers`;
          ...
          if (child.type === 1 /* ELEMENT */ &&
              ...
              if (isStaticNode(child, resultCache)) {
                  ...
                  if (!flag ||
                      ...
                      if (props && props !== `null`) {
                          child.codegenNode.arguments[1] = context.hoist(props);
                      }
                  }
              }
          }
          if (child.type === 1 /* ELEMENT */ || child.type === 11 /* FOR */) {
  ...
  function finalizeRoot(root, context) {
      ...
      if (children.length === 1) {
          ...
          if (child.type === 1 /* ELEMENT */ &&
              !isSlotOutlet(child) &&
              child.codegenNode &&
              child.codegenNode.type === 12 /* JS_CALL_EXPRESSION */) {
  ...
  function createCodegenContext(ast, { mode = 'function', prefixIdentifiers = false, sourceMap = false, filename = `template.vue.html` }) {
      const context = {
          mode,
          prefixIdentifiers,
          sourceMap,
  ...
  function createChildrenCodegenNode(branch, index, context) {
      ...
      if (needFragmentWrapper) {
          ...
          if (children.length === 1 && child.type === 11 /* FOR */) {
              ...
              const forBlockArgs = child.codegenNode.expressions[1]
                  .arguments;
  ...
  const toValidId = (str) => str.replace(/[^\w]/g, '');
          ...
          if (node.tagType === 0 /* ELEMENT */ ||
              ...
              return () => {
                  ...
                  if (isComponent) {
                      ...
                      context.statements.add(`const ${componentIdentifier} = ${context.helper(RESOLVE_COMPONENT)}(${JSON.stringify(node.tag)})`);
                  }
                  const args = [
  ...
  function createDirectiveArgs(dir, context) {
      ...
      context.statements.add(`const ${dirIdentifier} = ${context.helper(RESOLVE_DIRECTIVE)}(${JSON.stringify(dir.name)})`);
      const dirArgs = [dirIdentifier];
      const { loc } = dir;
      if (dir.exp)
